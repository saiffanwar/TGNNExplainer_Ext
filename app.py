...
parser.add_argument('-em', '--expmode', type=str, default='fidelity', help='Mode for the simulated annealing algorithm')
parser.add_argument('-d', '--dataset', type=str, default='synth_junction', help='Mode for the simulated annealing algorithm')
args = parser.parse_args()

results_dir = f'results/{args.dataset}/best_result_{args.target_node}_{args.subgraph_size}_{args.expmode}.pck'

with open(results_dir, 'rb') as f:
    explainer, sa = pck.load(f)

exp_vis_fig, plotting_data = graph_visualiser(args.dataset, explainer, sa)
exp_heatmap_fig = explanation_heatmap(args.dataset, plotting_data, sa)
