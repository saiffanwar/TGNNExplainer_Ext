...
if [ "$1" == "tgnne" ]; then
    python3.12 ~/TGNNExplainer_Ext/benchmarks/explainer_run/run.py datasets=${dataset} device_id=0 explainers=tgnnexplainer models=${model} results_batch=${2}
fi
...
if [ "$1" == "temp_me" ]; then
    python3.12 ~/TGNNExplainer_Ext/benchmarks/explainer_run/run.py datasets=${dataset} device_id=0 explainers=temp_me models=${model}
fi
